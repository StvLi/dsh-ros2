...
export function createRos2Tools(deps: ToolDeps) {
  ...
  tools.push(makeMotionValidateTool(deps))
...
// ── pre-execution motion validation (motion_validator.py) ─────
// Contract: docs/safety-todo.md early batch 0.14.1 — deterministic checks
// (limits/NaN/names/freshness/workspace box/fingerprint/TTL) run BEFORE human
// approval; collision/singularity stay with MoveIt planning.

interface ProfileSafetyView {
  safety?: Record<string, unknown>
  joints?: Array<{ name: string; limits?: Record<string, unknown> }>
  moveit?: { groups?: Record<string, { joints?: string[] }> }
}

async function loadRobotProfile(deps: ToolDeps, robot: string): Promise<{ robot: ProfileSafetyView; profile_path?: string } | null> {
  const res = await deps.run('python3', [scriptPath('robot_profile.py'), 'load', '--name', robot], { timeoutMs: 30000 })
...
/** Parse "name:=value name:=value" into {name: number}. */
...
/** Parse "x y z rx ry rz" into {x,y,z,rx,ry,rz}. */
...
/**
 * L2: unified MoveIt motion — plan → validate → approve (validation shown) →
 * execute → verify, the single motion path (docs/safety-todo.md early batch
 * 0.14.1). Generic: standard moveit_msgs + SRDF, never a specific package.
