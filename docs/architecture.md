...
┌───────────────▼─────────────────────────────────────────┐
│ 工具层 src/tools.ts（30 个工具）                          │
│  L1 只读诊断 ×17 │ L2 管理 ×7 │ L3 可视化 ×6              │
└──────┬──────────────┬──────────────┬──────────┬─────────┘
       │              │              │          │
  ┌────▼────┐   ┌─────▼─────┐  ┌─────▼────┐  ┌──▼─────────┐
...
| `gui` | L3 GUI 生命周期 + 截图 | `GuiManager`（spawn/wmctrl/Pillow） | fake spawn/windowCmd/screenshot |
...
- L3：本地 GUI 进程与截图属良性操作，无审批；视觉 API key 用户自备（`vision.apiKey`），不落盘明文
