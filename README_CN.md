...
> **提示词引导**：`dsh-ros2-core` 会向 system prompt 注入一段分节，引导模型用本
> 工具链做 ROS2 相关工作——用 `ros2_*` 发现图、用 `robot_load`/`robot_topology`
> 载入机体、用 `moveit_*`/`robot_safety_*` 规划与执行（均需审批，并以
> `/safety/state` 为闸门）——而不是临时拼 `ros2` CLI 或私自写 `rclpy` 脚本。
> 因此**凡是装了 core 的安装形态都带它**（含 `dsh-ros2` 聚合包）。
>
> 该分节在**每次组装的当下**依据已注册的工具重建，所以只装 core 时不会被引导去调用
> 它并未随附的 `moveit_move`/`robot_safety_*`。它与插件**同生命周期**注册：
> 禁用/卸载插件即从提示词中移除，不会留下残留。
...
| `ros2_topic_find` | `ros2 topic find <type>` | 携带某消息类型的话题 |
...
| `ros2_action_info` | `ros2 action info <action>` | 动作类型与状态 |
| `ros2_action_type` | `ros2 action type <action>` | 动作类型 |
| `ros2_param_list` | `ros2 param list <node>` | 节点参数 |
| `ros2_param_get` | `ros2 param get <node> <param>` | 读取单个参数值 |
| `ros2_param_dump` | `ros2 param dump <node>` | 转储节点全部参数 |
...
| `ros2_param_delete` | `ros2 param delete <node> <param>` | 删除节点参数（审批门控） |
| `ros2_lifecycle` | `ros2 lifecycle get|list|set <node> [state]` | 生命周期状态 get/list（L1）或 set（L2 审批） |
| `ros2_component` | `ros2 component list` / `load <container> <pkg> <type>` | 组件容器 list（L1）/ load（L2 审批） |
...
安装需 **pnpm 11.x**（仓库固定 `packageManager: pnpm@11.22.0`；CI 通过 `pnpm/action-setup@v6` 安装匹配版本）。
Node 需 `^22.19 || >=24`。

```bash
pnpm install
pnpm run typecheck   # tsc --noEmit
pnpm run test        # vitest（292 例；另有 10 个 sidecar Python 场景 + 6 个 zero-pose 自检 + 29 项档案名/路径/TF 解析自检）
